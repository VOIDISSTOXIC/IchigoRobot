...
from Ichigochan.modules.helper_funcs.chat_status import dev_plus
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "Lockdown is " + "on" if not Ichigochan.ALLOW_CHATS else "off"
        update.effective_message.reply_text(f"Current state: {state}")
        return
    if args[0].lower() in ["off", "no"]:
        Ichigochan.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        Ichigochan.ALLOW_CHATS = False
    else:
